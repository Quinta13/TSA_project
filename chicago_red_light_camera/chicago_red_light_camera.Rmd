...
2. A first investigation involves **decomposing the time series into seasonality and trend**, using various filtering techniques. While it is common for the number of violations to be influenced by seasonal factors, this also reveals a substantial trend that could indicate certain tendencies the area is subject to and may be addressed through policies to reduce violations.

3. A second analysis explores the **potential different nature of violations between weekdays and weekends**. Considering that people usually have different lifestyles on these two days, mainly due to work-related reasons, it makes sense to investigate if this is reflected in the data and if separate modeling is warranted.
...
```{r df violations boxplot, fig.width=6, fig.height=5}
boxplot(
...
```{r camera histogram, fig.width=5, fig.height=5}
barplot(
  camera.df$Camera,
...
```{r mean violation per camera histogram, fig.width=5, fig.height=5}
...
The graph highlights a probable tendency to commit more violations in the two southern regions, *South* and *FarSouthEast*, and a lower number in the *FarNorth*, and especially in the *NorthWest*.
...
```{r chicago regions, fig.cap="Chicago Regions", out.width = "300px"}
...
For this reason, in the next section, we suggest a **second step of aggregation by grouping the Regions into three Areas** based on geographical proximity and similar patterns found in analyses.
...
```{r chicago sides,  fig.cap="Chicago Areas", out.width = "500px"}
...
```{r area daily ts grid, fig.width=5, fig.height=7}
plot_ts_grid(
...
The simultaneous visualization now finally provides some insights. It is particularly evident that while the trend of *North* and *Center* remains quite stable,  the *South* has a noticeable trend: in the beginning of the time series its observation are lower than the other two Area, but in the post-pandemic years is has a steep increase and to reach levels similar to those of the other two.
...
```{r area weekly ts grid, fig.width=5, fig.height=7}
plot_ts_grid(
...
The combined graph even better highlights the upward trend in the *South* area. It also now capable of showing that, although the *North* and *Center* areas have a very similar overall curve, the one in the *Center* reaches much higher peaks during the summers.
...
Differently from the other two areas, for the South, many outliers have been identified, all in the second half of the time series when there was a significant increase in the average number of violations following the pandemic years. Some points may not be considered outliers in all respects and might be the result of some limitations in the procedure; therefore, we only retain modifications for some of them:
...
```{r area daily ts grid new, fig.width=5, fig.height=7}
plot_ts_grid(
  ts_list=area.ts.daily,
  n_row=3,
  names=names.area,
...
```{r area weekly ts grid new, fig.width=5, fig.height=7}
plot_ts_grid(
  ts_list=area.ts.weekly,
  n_row=3,
  names=names.area,
...
The results are very similar to the previous version, although this new one perhaps offers a **more appreciable version of the annual smoothing** in the *North* and *Center* areas, where there is a noticeable increase in *2017* followed by a decline that extended until the pandemic years, with then return to an upward phase after *2021*.
...
```{r daily weekly monthly quarterly deseasoning, fig.width=8, fig.height=8}
par(mfrow = c(length(area.ts.daily), 1))
...
for(area_name in names.area) {
  ...
  plot_deseasoning_varying_freq(
    ts=daily,
    main=paste(area_name, "- Multiple deseasoning of daily observations"),
    freq = c(7, 30, 90),
...
Deseasoning in **daily** observations.
...
Deseasoning in **weekly** observations.
...
for(area_name in names.area) {
  ...
  plot(
    weekly,
    main=paste(area_name, "- Yearly deaseasoning of weekly Deseasoning"),
    ylab=names.ylab$weekly,
    col="dimgray"
  )
...
* the ***Central*** area generally shows similar trends to the *North*, but it is important to note the possible absence of the hump shape present in the pandemic years, which is here only present in the months from *March* to *November*.
...
For the *Center* area, both the trend and seasonality seem to contribute equally with an amount of 250 observations, while the erratic component is contained in a range of about 100.
...
The result evidence the **dependency between weekdays and weekends**, which we can conclude to be **linear**. The model works particularly well for the *South* area, which has a multiple R-squared that exceeds $0.9$. This type of visualization also provides another view of the number of violations for the *South* area, which we know has undergone a mean increase in recent years, highlighted by the different density of two point clouds in the graph.
...
Finally, let's leverage an **automatic fit** with parameters generated automatically by the software with `auto.arima()` using *AICc* as the criterion for the best fit selection.
...
The **best fitted model** is the one resulted by the **automatic parameter selection**. Let's inspect the behavior of it's residuals.

```{r daily best fit diagnostic, fig.width=7, fig.height=5}
tsdiag(fit.daily$Center$best, main="Daily fit - Residuals")
```

The **residuals are not satisfactory**, as the Ljung-Box test suggests some autocorrelation, and they do not exhibit characteristics consistent with white noise. The model is **not of good quality**, but this is also partly due to the challenging task of modeling data with very high variability, such as daily observations.
...
```{r daily forecasts, fig.width=10, fig.height=8}
par(mfrow=c(2, 2))
...
The model provides a **highly variable prediction and only for a very short time window**, and then predicts a mean value with a variability that becomes extremely large. The expected errors are very high.

```{r daily forecasts errors}
print(errors$daily)
```

This outcome is expected, as this type of models are **typically reliable when making short-term forecasts**. Their effectiveness tends inevitability to diminish when applied to longer time horizons.

### One-step-ahead Forecasting

We try to overcome this limitation devising a second evaluation strategy: we perform **one-step-ahead forecasting for each instance in the test set**, meaning a single daily prediction using the information available up to the day before.
...
The prediction manages to follow the general trend of the data. However, having access to the entire information up to the day before, the **accuracy is not so precise**, and the **variability is also quite high**, confirming the low quality of the model. 

This type of evaluation allows us to determine **for which days the model makes the best and worst predictions**.
...
The plot is **not very interpretable** since it alternates between highs and lows throughout the year. The only noteworthy information is that three of the dates with the highest errors occur around the New Year when a period brings unpredictability that is challenging for the model to capture.

In conclusion, **daily modeling is ineffective and poorly reliable**. Perhaps predicting the individual daily value may be not truly the focus of real-word application: it might be more interesting and useful to **predict an average weekly trend**, which is what we aim to do in the next chapter.
...
As we have previously done with the daily forecasting, we inspect **different trasformation of the original time series** to devise different fitting models.
...
```{r weekly forecasts, fig.width=10, fig.height=8}
par(mfrow=c(2, 2))
...
The prediction interval in quite wide for any prediction horizon. The one-month and three-month point predictions adhere to the true trend, while there is an underestimation for the last six months and an overestimation for the last month. This observation is also reflected in the errors made.
...
The weeks in April and July stand out as having lower errors, suggesting potentially better predictive capabilities for the model during these periods. Conversely, the model performs poorly especially on the last two weeks of May; looking at the trend can suspect that an unexpected event may have occurred, leading to increased traffic during that time. Similarly to what happened with daily predictions even year the last week of year.
...
```{r north ccf, fig.height=5, fig.width=7}
ccf(
...
```{r south ccf, fig.height=5, fig.width=7}
ccf(
...
```{r north lr, fig.height=5, fig.width=7}
plot(
  x=as.numeric(area.ts.weekly$North),
  y=as.numeric(area.ts.weekly$Center),
...
## Conclusions
No much sense, basiclly same inforamtion
